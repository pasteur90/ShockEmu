...
- (void) start {
  zmq_ctx = zmq_ctx_new();
  subscriber = zmq_socket(zmq_ctx, ZMQ_SUB);
  int rc = zmq_connect(subscriber, "tcp://localhost:54321");
  assert(rc == 0);
...
+(void)gpadloop:(id)param{
  ...
  while (true) {
    int rc = zmq_recv(subscriber, buffer, 2, 0);
    if (rc != 2) continue;
    [hid tickpad:buffer[0] :buffer[1]];
  } 
}
@end
...
- (void)tickpad: (int)code :(int)val {
  ...
  dpadUp    = (code == 5 && val == 1 ? 1 : 0);
  dpadRight = (code == 6 && val == 1 ? 1 : 0);
  dpadDown  = (code == 7 && val == 1 ? 1 : 0);
  dpadLeft  = (code == 8 && val == 1 ? 1 : 0);
  ...
  switch (code) {
  case 1: share     = onOff; break;
  case 2: L3        = onOff; break;
  case 3: R3        = onOff; break;
  case 4: options   = onOff; break;
  case 9: L2        = onOff; break;
  case 10: R2       = onOff; break;
  case 11: L1       = onOff; break;
  case 12: R1       = onOff; break;
  case 13: triangle = onOff; break;
  case 14: O        = onOff; break;
  case 15: X        = onOff; break;
  case 16: square   = onOff; break;
  case 17: touchpad = onOff; break;
  case 48: uleftX   = val; break;
  case 49: uleftY   = val; break;
  case 50: urightX  = val; break;
  case 53: urightY  = val; break;
  default:;
  }
